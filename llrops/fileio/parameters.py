...
class ParameterVector:
    ...
    def __post_init__(self) -> None:
        ...
        if sigmas is not None:
            sigmas = np.array(sigmas, dtype=float, copy=True).reshape(-1)
            if (
                sigmas.size != values.size
                or not np.all(np.isfinite(sigmas))
                or np.any(sigmas < 0.0)
            ):
                raise ValueError(
                    "Parameter sigmas must be finite, non-negative, and aligned."
                )
...
def write_parameter_vector(vector: ParameterVector, path: str | Path) -> Path:
    ...
    with atomic_text_writer(target, "parameterVector") as stream:
        ...
        stream.write(f"hasSigma {1 if vector.sigmas is not None else 0}\n")
        stream.write("# parameter_name unit value sigma\n")
        stream.write("data\n")
        for index, (name, unit, value) in enumerate(
            zip(vector.parameter_names, vector.units, vector.values)
        ):
            sigma = "~" if vector.sigmas is None else format_float(vector.sigmas[index])
            stream.write(
                f"{encode_token(name)} {encode_token(unit)} {format_float(value)} {sigma}\n"
            )
    return target
...
def read_parameter_vector(path: str | Path) -> ParameterVector:
    ...
    with open_text_reader(source) as stream:
        ...
        if (
            ...
            or len(sigma_line) != 2
            or sigma_line[0] != "hasSigma"
            or marker != "data"
        ):
            raise ValueError(f"Malformed parameter-vector header in {source}.")
        count = int(count_line[1])
        if count < 0 or sigma_line[1] not in {"0", "1"}:
            raise ValueError(
                f"Invalid parameter-vector count or hasSigma flag in {source}."
            )
        has_sigma = sigma_line[1] == "1"
        names: list[ParameterName] = []
        units: list[str] = []
        values: list[float] = []
        sigmas: list[float] = []
        for line in lines:
            ...
            if has_sigma:
                if fields[3] == "~":
                    raise ValueError(
                        "Parameter-vector sigma is missing despite hasSigma=1."
                    )
                sigmas.append(parse_float(fields[3], field="parameter sigma"))
            elif fields[3] != "~":
                raise ValueError(
                    "Parameter-vector sigma is present despite hasSigma=0."
                )
    if len(names) != count:
    ...
    return ParameterVector(
        ...
        np.asarray(values),
        tuple(units),
        None if not has_sigma else np.asarray(sigmas),
        decode_token(kind_line[1]),
    )
