...
def _validate_adjustment_state(payload: Mapping[str, object]) -> None:
    ...
    if (
        ...
        raise ValueError(
            "Adjustment state fingerprint must be a lowercase SHA-256 digest."
        )
    ...
    for key, values in payload["reflectorPositions"].items():
        position = np.asarray(values, dtype=float)
        if position.shape != (3,) or not np.all(np.isfinite(position)):
            raise ValueError(f"Adjustment state reflector position {key!r} is invalid.")
    scales = np.asarray(list(payload["scales"].values()), dtype=float)
    factors = np.asarray(list(payload["robustFactors"].values()), dtype=float)
    ...
    if not np.all(np.isfinite(factors)) or np.any((factors < 0.0) | (factors > 1.0)):
        raise ValueError(
            "Adjustment state robust factors must be finite and in [0, 1]."
        )
