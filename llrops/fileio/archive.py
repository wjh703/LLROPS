...
Native LLROPS artifacts identify their scientific type in the first line.  The
...
def require_text_path(path: str | Path) -> Path:
    ...
    if not is_text_path(target):
        raise ValueError(f"LLROPS text archives require .txt or .txt.gz: {target}")
    return target
...
def require_binary_path(path: str | Path) -> Path:
    ...
    if not is_binary_path(target):
        raise ValueError(f"LLROPS binary archives require .dat or .dat.gz: {target}")
    return target
...
def decode_token(value: str) -> str:
    ...
    if _PERCENT_ESCAPE.search(value):
        raise ValueError(f"Invalid percent escape in LLROPS token {value!r}.")
    try:
        return unquote_to_bytes(value).decode("utf-8", errors="strict")
    except UnicodeDecodeError as exc:
        raise ValueError(f"Invalid UTF-8 in LLROPS token {value!r}.") from exc


def format_float(value: object) -> str:
    number = float(value)
    if number != number or number in (float("inf"), float("-inf")):
        raise ValueError(f"LLROPS archives reject non-finite float {value!r}.")
...
def write_header(stream: TextIO, artifact_type: str) -> None:
    if _ARTIFACT_TYPE.fullmatch(str(artifact_type)) is None:
        raise ValueError(f"Invalid LLROPS artifact type {artifact_type!r}.")
    stream.write(f"llrops {artifact_type} version={ARCHIVE_VERSION}\n")
...
def parse_header(stream: TextIO, expected_type: str | None = None) -> str:
    for raw in stream:
        ...
        if (
            ...
            raise ValueError(f"Invalid LLROPS archive header: {text!r}")
        try:
            version = int(parts[2].split("=", 1)[1])
        except ValueError as exc:
            raise ValueError(f"Invalid LLROPS archive version in {text!r}") from exc
        if version != ARCHIVE_VERSION:
            raise ValueError(
                f"Unsupported LLROPS archive version {version}; expected {ARCHIVE_VERSION}."
            )
        ...
        if expected_type is not None and artifact_type != expected_type:
            raise ValueError(
                f"Expected LLROPS {expected_type!r} archive, found {artifact_type!r}."
            )
        return artifact_type
    raise ValueError("LLROPS archive is empty.")
...
def open_text_reader(path: str | Path) -> ContextManager[TextIO]:
    ...
    if not target.is_file():
        raise FileNotFoundError(f"LLROPS text archive not found: {target}")
...
def open_binary_reader(path: str | Path):
    target = require_binary_path(path)
    if not target.is_file():
        raise FileNotFoundError(f"LLROPS binary archive not found: {target}")
...
def require_file_group_path(path: str | Path) -> Path:
    ...
    if target.suffix:
        raise ValueError(
            f"LLROPS file groups require an extensionless directory: {target}"
        )
    return target
