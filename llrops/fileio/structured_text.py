...
def plain_data(value):
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            ...
            if not normalized_key:
                raise ValueError("Structured LLROPS text rejects empty mapping keys.")
            if normalized_key in result:
                raise ValueError(
                    f"Structured LLROPS text has colliding mapping key {normalized_key!r}."
                )
    ...
    if value is None or isinstance(value, (str, int, float, bool)):
        if isinstance(value, float) and not np.isfinite(value):
            raise ValueError("Structured LLROPS text rejects non-finite floats.")
        return value
    raise TypeError(
        f"Structured LLROPS text cannot encode {type(value).__name__} objects."
    )
...
def read_structured_text(path: str | Path, artifact_type: str) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"LLROPS {artifact_type} payload must be a mapping: {source}")
    return payload
