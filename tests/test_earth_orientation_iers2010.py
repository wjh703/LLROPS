...
def test_libration_corrections_match_iers_reference_values():
    polar_motion = earth_rotation_libration_eop_correction(
        Epoch(2_400_000.5, 54_335.0, TimeScale.TT)
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        earth_rotation_libration_eop_correction(
            Epoch(2_400_000.5, 44_239.1, TimeScale.TT)
        ).libration_delta_ut1_s * 1.0e6,
    ...
    np.testing.assert_allclose(
        earth_rotation_libration_eop_correction(
            Epoch(2_400_000.5, 55_227.4, TimeScale.TT)
        ).libration_delta_ut1_s * 1.0e6,
