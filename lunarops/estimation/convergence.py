...
class ParameterConvergencePolicy:
    ...
    def __post_init__(self) -> None:
        if not isfinite(float(self.default_tolerance_m)) or self.default_tolerance_m < 0.0:
            raise ValueError("Default parameter tolerance must be finite and non-negative.")
        if any(not isfinite(float(value)) or float(value) < 0.0 for value in self.tolerance_by_block_m.values()):
    ...
    def evaluate(self, updates_m: Mapping[str, float]) -> ParameterConvergenceEvaluation:
        if any(not isfinite(float(value)) or float(value) < 0.0 for value in updates_m.values()):
